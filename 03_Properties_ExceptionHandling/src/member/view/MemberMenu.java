...
public class MemberMenu {
	...
	public void mainMenu() {
		...
		while(true) {
			...
			case "6":
				...
				break;
			case "7":
				delList = memberController.findAllMemberDel();
				printMemberDelList(delList);
				break;
			case "0": return;
			default:
	...
	/**
     * 여러 행 조회 결과 출력 메소드(현재 가입 회원 조회용)
     * (list가 null인 경우 없음)
     * - 0행 : empty list가 리턴됨
	...
	public boolean checkId(String id) {
		Member tmp = memberController.findMemberById(id);
		if(tmp != null) {
			System.out.println("> 사용 불가능한 아이디입니다. 다시 입력하세요.");
			return false;
		}
		else {
			System.out.println("> 사용 가능한 아이디입니다. 회원가입을 계속 진행합니다.");
			return true;
		}
	}
	 ...
	 * @return
	 */
	private String inputUpdateData(Object rawData) {
	...
	/**
     * 여러 행 조회 결과 출력 메소드(탈퇴 회원 조회용)
     * @param list
     */
    public void printMemberDelList(List<MemberDel> delList) {
        if(delList == null || delList.isEmpty()) {
            ...
            System.out.println("-----------------------------------------------------");
            for(MemberDel memberDel : delList) {
                System.out.printf("%15s%15s%15s%15s%15s%15s%15s%15s%n", 
                        memberDel.getId(), 
                        memberDel.getName(),
                        memberDel.getGender(),
                        memberDel.getBirthday(),
                        memberDel.getEmail(),
                        memberDel.getAddress(),
                        memberDel.getRegDate(),
		                memberDel.getDelDate());
            }
            System.out.println("-----------------------------------------------------");
            System.out.println();
        }
